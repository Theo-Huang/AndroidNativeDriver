...
public class AdbConnectionBuilder {
	...
	@Nullable
	private Integer adbServerPort;
	@Nullable
	private Integer emulatorConsolePort;
	@Nullable
	private Integer emulatorAdbPort;
	...
	/**
	 * Attempts to find {@code adb} automatically by reading the SDK path from {@code ANDROID_SDK}
	 * environment variable and searching certain sub-directories in it. Then it sets the ADB path of
	...
	/**
	 * Attempts to find {@code adb} automatically given the Android SDK path and then searching
	 * certain sub-directories in it. Then it sets the ADB path of this builder to that value.
	 * 
	...
	private String findAdb(String androidSdkPath) {
		...
		for (String subDirectory : possibleLocations) {
			String fullPath = joinPath(androidSdkPath, new String[] { subDirectory, "adb" });
			if (fileExists(fullPath)) {
				return fullPath;
			} else if (fileExists(fullPath + ".exe")) {
				return fullPath + ".exe";
			}
