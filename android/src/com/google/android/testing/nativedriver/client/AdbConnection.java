...
/**
 * Encapsulates actions that can be accomplished with the Android Debug Bridge (ADB) or the Android
 * shell commands (such as {@code adb shell cat FILENAME}). By using this class to invoke
 * {@code adb}, you can customize the path to {@code adb} and override the port numbers used by
 * {@code adb}. (See {@link #AdbConnection(File, Integer, Integer, Integer)} for more information).
 * 
 * <p>
 * For readability purposes, the configuration and construction of instances of this class are
 * performed with {@code AdbConnectionBuilder}.
 * 
 * @author Matt DeVore
 * @author Gagan Gupta
 * @author Theo Huang
...
public class AdbConnection {
	...
	@Nullable
	private final Integer emulatorConsolePort;
	@Nullable
	private final Integer emulatorAdbPort;
	...
	public ArrayList<String> getTestableAndroid() {
	...
	/**
	 * Performs the {@code ioctl} command on device corresponding to the given filename. This is
	 * equivalent to {@code adb shell ioctl -rl (buffer.length) (filename) (requestCode)}.
	 * 
	 * @param filename
	 *          the name of the device file to perform the request on
	 * @param requestCode
	 *          the request code to perform
	 * @param length
	 *          the amount of bytes to expect from the request
	 * @return a byte array the size of {@code length} that contains the results of the request
	 */
	...
	/**
	 * Gets the contents of a file or device on the Android device. This is equivalent to running
	 * {@code adb pull (FILENAME) /dev/stdout} on the command line.
	 * 
	 * @param filename
	 ...
	 * 
	 * @param text
	 *          to send to the device/emulator
	 */
	...
	/**
	 * Runs {@code adb} using the given arguments and under the configuration values passed to the
	 * constructor.
	 * 
	 * @param arguments
	 *          the arguments to pass to the {@code adb} utility
	...
	/**
	 * Reads from the given stream until end-of-file is reached. All data read is ignored. This is
	 * useful for closing a process gracefully when there are unread data in standard out that you
	 * want to ignore.
	 * 
	 * @throws AdbException
	 *           if an {@link IOException} occurred while reading from the stream. If
	 *           {@code inputStream} does not represent the output of some ADB process, then this
	 *           exception will be misleading.
	 */
	...
	/**
	 * Constructs a new instance which uses {@code adb} from a particular path and possible overriding
	 * of configurable ports.
	 * 
	 * <p>
	 * For readability purposes, the configuration and construction of instances of this class should
	 * be performed with {@link AdbConnectionBuilder}.
	 * 
	 * @param adbPath
	 *          the path to the {@code adb} utility
	 * @param adbServerPort
	 *          the port that the ADB daemon listens on to serve requests. If {@code null}, the
	 *          default port (possibly 5037) is used.
	 * @param emulatorConsolePort
	 *          the port the emulator is listening on for console commands. If {@code null}, the
	 *          default port (possibly 5554) is used.
	 * @param emulatorAdbPort
	 *          the port the emulator is listening on for ADB commands. If {@code null}, the default
	 *          port (possibly 5555) is used.
	 */
	protected AdbConnection(String adbPath,
		...
		this.bridge = AndroidDebugBridge.createBridge(adbPath, false);
	}

	// TODO Try to find some method to create testing grid environment with adb -s <device ID>.
	// ddmlib may help this.
	//
	// May use these to get screenshot
	...
	// http://androidsnippets.wordpress.com/2012/10/25/how-to-convert-bitmap-to-byte-array-in-android-and-java/
