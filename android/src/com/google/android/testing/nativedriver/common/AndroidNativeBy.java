...
public abstract class AndroidNativeBy extends By {
  ...
  public static AndroidNativeBy all(final boolean currentActivity) {
    return new AndroidNativeBy() {
      @Override
      public WebElement findElement(SearchContext context) {
      	throw new UnsupportedOperationException("Only for all(findElements) now.");
      }

      @Override
      public List<WebElement> findElements(SearchContext context) {
      	if(currentActivity)
      		return ((FindsAll) context).findAllElements();
    		else
    			return ((FindsAll) context).findAllElementsByCurrentActivity();
      }

      @Override
      public String toString() {
        return "AndroidNativeFindAll.currentActivity: " + currentActivity;
      }
    };
  }
}
