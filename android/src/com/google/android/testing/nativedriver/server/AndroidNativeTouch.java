...
public class AndroidNativeTouch implements Touch {
	...
	private static final long DEFAULT_WAIT_SYNC_SLEEP_INTERVAL = 200;
	private static final long DEFAULT_WAIT_SYNC_TIMEOUT = 2000;
	private final AndroidWait androidWait =
			new AndroidWait(
					new AndroidSystemClock(),
					DEFAULT_WAIT_SYNC_SLEEP_INTERVAL,
	...
	protected void sendMotionEvent(MotionEvent motionEvent, boolean releaseTouchStateOnDrop) {
		final Thread waitSync = new Thread() {
			@Override
			public void run() {
		...
		try {
			androidWait.until(new Function<Void, Boolean>() {
				...
				public Boolean apply(Void input) {
					return !waitSync.isAlive();
				}
			});
		} catch (TimeoutException exception) {
			System.err.println("Drop current event.");
			if (releaseTouchStateOnDrop) {
				System.err.println("Set Touch State Released");
				setTouchStateReleased();
			}
		}
		try {
