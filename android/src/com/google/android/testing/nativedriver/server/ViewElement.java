...
import com.google.android.testing.nativedriver.common.AndroidElementAttributeConstant;
 ...
 * view-wrappers, and is used
 * un-extended only when a more specific view-wrapper could not be found.
 * 
 * <p>
 * This class has support for operations that are the same for all {@code View}s,
 * such as {@code sendKeys}, which only requires requesting focus and then sending
 * keys through the test {@code Instrumentation}.
 * 
 * <p>
 * Note that this wrapper does not support searching child views by ID, even though
 * the {@code View} class has a {@code findViewById} method. (In order to support
 * searching child views, a class should override
 * {@code AndroidNativeElement.findElementByAndroidId}). This functionality is not
 * needed in this class, since only {@code ViewGroup} and other {@code View}
 * subclasses should have children views, and the corresponding {@code ViewElement}
 * subclasses have support for enumerating child views.
 * 
 * @param <V>
...
public class ViewElement<V extends View>
   ...
   * wrapped {@code View} is an instance of it. For instance, if {@code className} is
   * {@code "android.widget.TextView"},
   * then this method will return {@code true} if the wrapped {@code View} is an
   * instance of {@link android.widget.TextView} or {@link android.widget.Button} .
   */
  @Override
  ...
  /**
   * Returns the literal ID of the wrapped {@code View}, or {@code null} if it does
   * not have one.
   * This implementation will only return non-null if this {@code View} is the decor
   * view of the
   * currently-focused {@code Activity}, in which case it returns
   * {@code LITERALID_FOCUSEDACTIVITY}.
   */
  @Nullable
  @Override
  protected String getLiteralId() {
    if (view == decorViewOfCurrentActivity()) {
      ...
      //      return null;
      return getAttribute(AndroidElementAttributeConstant.LITERAL_ID);
    }
  }

  /**
   * Returns the ID of the wrapped {@code View}, or {@code null} if it does not have
   * an ID.
   */
  @Nullable
  @Override
   ...
   * {@inheritDoc}
   * 
   * This implementation returns {@code true} if the wrapped {@code View} does not
   * have window
   * focus.
   * 
   ...
   * {@link #requestFocus()} and then sending the keys with the {@link KeySender} of
   * the {@code ElementContext}.
   */
  @Override
  ...
  public String getAttribute(String name) {
    if (name.equals(AndroidElementAttributeConstant.CONTEXT)) {
      return view.getContext().getClass().getName();
    } else if (name.equals(AndroidElementAttributeConstant.LITERAL_ID)) {
      Integer id = getAndroidId();
      if (id != null) {
        Context context = view.getContext();
        return context.getResources().getResourceEntryName(id);
      }
    } else if (name.equals(AndroidElementAttributeConstant.ANDROID_ID)) {
      return String.valueOf(getAndroidId());
    }
    return null;
  }
   ...
   * {@code ViewElement}. This is
   * done synchronously on the main thread of the application using the
   * {@link Runner} in the {@code ElementContext}. If we fail to get focus, issue a
   * click to trigger. Note that the actual
   ...
   * just enough. This is done synchronously on the main thread of the application
   * using the {@link Runner} in the {@code ElementContext}. If we fail to get focus,
   * issue a click to
   * trigger. Note that the actual focused {@code View} might not be the one
