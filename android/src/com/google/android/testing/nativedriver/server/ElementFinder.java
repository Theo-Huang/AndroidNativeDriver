...
/**
 * Encapsulates element search functionality. An instance of this class knows how to filter a
 * sequence of {@code AndroidNativeElement}s using any of the search methods supported by Android
 * NativeDriver.
 * 
 * <p>
 * The currently-supported methods of searching are explained in detail below.
 * 
 * <h3>Finding by ID ({@link FindsById})</h3>
 * 
 * <p>
 * All elements are assigned at most one of each kind of the following IDs:
 * <ul>
 * <li>An integer that is preceded by a {@code #}. The integer corresponds to the value returned by
 * {@code View.getId} or {@code MenuItem.getItemId}
 * <li>A string that corresponds to the string ID of an Android {@code View} or, more generally, a
 * field in {@code R.id}. This is converted to an integer value by reading the field through
 * reflection, the processed in the same way as the previous ID type
 * <li>A string that begins with {@code $} which acts as a <em>literal</em> ID
 * </ul>
 * 
 * <p>
 * Any Android view ID which is associated with more than one view should be used in a search from
 * some parent element that only contains one of the views, unless returning all views is desired.
 * For instance, if there are two views with ID "ambiguousId", but only one view is a descendant of
 * "parent", you can narrow your search with code like this:<br>
 * 
 * <pre>
 * androidNativeDriver.findElement(By.id(&quot;parent&quot;))
 * 		.findElement(By.id(&quot;ambiguousId&quot;))
 * </pre>
 * 
 * <p>
 * Note that not all elements are capable of enumerating their children (see the corresponding
 * {@code WebElement} implementation's JavaDoc). In this case, to find a child of element {@code x},
 * which does not support child enumeration, both {@code x} and its child must be searched for
 * explicitly. For instance, if element {@code x} is an options menu:<br>
 * 
 * <pre>
 * androidNativeDriver.findElement(By.id(&quot;$optionsMenu&quot;))
 * 		.findElement(By.id(&quot;child&quot;))
 * </pre>
 * 
 * @author Matt DeVore
...
public class ElementFinder {
	...
	private class SearchContextImpl
			implements SearchContext, FindsById, FindsByText, FindsByClassName, FindsAll {
		...
		private WebElement findElementByAndroidId(String using) {
			...
			if (androidId == null) {
				...
				return findElementFromHierarchy(
						scope.getChildren(), new ByAndroidIdFilterCondition(using, androidId));
		...
		public List<WebElement> findAllElements() {
			return addAllElementsFromHierarchy(Lists.<WebElement> newArrayList(),
					false, scope.getChildren(), Integer.MAX_VALUE);
		}

		@Override
		public List<WebElement> findAllElementsByCurrentActivity() {
			return addAllElementsFromHierarchy(Lists.<WebElement> newArrayList(),
					true, scope.getChildren(), Integer.MAX_VALUE);
		}
	}
	...
	private static List<WebElement> addAllElementsFromHierarchy(
			List<WebElement> destination, boolean currentActivity,
			Iterable<? extends AndroidNativeElement> topLevelElements, int maxResults) {
		for (AndroidNativeElement element : topLevelElements) {
			if (destination.size() >= maxResults) {
				break;
			}
			// true = current,false= all alive
			if (currentActivity) {
				if (!element.shouldOmitFromFindResults()) {
					destination.add(element);
				}
			} else {
				destination.add(element);
			}
			addAllElementsFromHierarchy(
					destination, currentActivity, element.getChildren(), maxResults);
		}
		return destination;
	}
	...
	/**
	 * Converts the given ID (such as {@code TextView01}) to an integral Android ID. This default
	 * implementation uses the {@code RClassReader} passed to the constructor and reads the value of
	 * the ID from the {@code id} inner class.
	 * 
	 * @param id
	 *          the string ID to convert
	 * @return the Android ID corresponding to the given string ID, or {@code null} if the given ID is
